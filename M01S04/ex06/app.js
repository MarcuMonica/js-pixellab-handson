...
console.warn(`
In mod similar, afiseaza skillurile care NU
incep cu j.
`);
for (var i = 0; i < person.skills.length; i++) {
  var skill = person.skills[i];
...
console.warn(`
Folosind un for afiseaza propozitia: "Prietenii mei
se numesc xxx yyy, xxx yyy, xxx yyy."
`);
...
console.warn(`
Folosind un for, afiseaza suma anilor de nastere
ale persoanelor.
`);
...
prietenii din arrayul friends doar daca aceasta este
mai mare de 2 ani.
`);
for (var i = 0; i < person.friends.length; i++) {
  var friend = person.friends[i];
...
inversa elementele arrayului skills. Atentie, va trebui sa numeri invers, de la length
la 0.
`);
...
console.warn(`
Folosind un for afiseaza propozitia: "Numele de
