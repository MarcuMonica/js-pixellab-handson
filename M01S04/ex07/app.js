...
prietenilor, insa o data ce l-ai gasit pe steven,
opreste bucla cu ajutorul keywordului break si
...
prietenilor, doar daca numarul de caractere total al numelor lor este mai
mare de 13 sub forma de propozitii “Prenume Nume are mai mult de 13
caractere.”.
`);
for (var i = 0; i < friendsCount; i++) {
  var friend = friends[i];
...
bucla la primul surname care are numarul de caractere mai mare sau egal decat
9 si afiseaz-l intr-o propozitie de forma “M-am oprit la Nume Prenume.”.
`);
for (var i = 0; i < friendsCount; i++) {
  var friend = friends[i];
  var fullName = friend.surname + ' ' + friend.name;
  var longestName = friend.surname;

  if (longestName.length >= 9) {
    console.log('M-am oprit la ' + fullName);
    break;
  }
  console.log(fullName);
